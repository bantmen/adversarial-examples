...
def parse_args():
    ...
    parser.add_argument('--is_verbose', default=True, help='Whether logging is verbose or not.')
    parser.add_argument('--victim_class', default=2, help='Victim class of the adversarial attack.')
    parser.add_argument('--wanted_class', default=6, help='Class to misclassify the victim samples for.')
    parser.add_argument('--num_iterations', default=30, help='Maximum number of iterations used for optimizing adversarial examples.')
    parser.add_argument('--num_viz_samples', default=10, help='Number of samples to visualize.')
...
def save_visualization(victim_x, modified_x):
    ...
    gs.tight_layout(fig)
    plt.savefig('adv_grid_viz.png')

MODEL_PATH = 'data/mnist_conv/mnist_conv'
args = parse_args()

maybe_print = print if args.is_verbose else lambda *args, **kwargs: None

maybe_print('Will create adversarial samples of {} to be misclassified as {}'.format(args.victim_class, args.wanted_class))
